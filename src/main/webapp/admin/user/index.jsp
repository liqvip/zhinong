		...
		<div class="panel-group" id="accordion" role="tablist" aria-multiselectable="true">
			...
			<div class="panel panel-primary">
				...
				<div id="collapseone" class="panel-collapse collapse in" role="tabpanel" aria-labelledby="headingone">
					<ul class="list-group">
						<li class="list-group-item"><a href="">用户查看</a></li>
		...
		$.ajax({
			...
			success:function(data,status,jqXHR){
			   ...
			   for(var i=0;i<data.list.length;i++) {
			      // 表格拼接
			      tableStr+="<tr>" +
					  "<td><img src='../images/user/"+data.list[i].userPhoto+"' alt=''></td>" +
					  "<td>"+data.list[i].loginName+"</td>" +
					  "<td>"+data.list[i].loginPwd+"</td>" +
                      "<td>"+data.list[i].userName+"</td>" +
					  "<td>"+data.list[i].userMail+"</td>" +
                      "<td>"+data.list[i].payPwd+"</td>" +
					  "<td>"+data.list[i].createTime+"</td>" +
					  "<td><a href='' data-toggle='modal' data-target='#deletemodal' id='delete' code='"+data.list[i].userId+"'>删除</a></td>"+
					  "<td><a href='' data-toggle='modal' data-target='#modifymodal' id='modify' code='"+data.list[i].userId+"'>修改</a></td>"+
					  "</tr>";
			   }
...
<script>
    ...
    $(document).on("click","#modify",function (event) {
        ...
        var userId = userObj.attr("code");
        console.log(userId);
		var loginNameObj = userObj.parent().siblings(":eq(1)");
        var loginPwdObj = userObj.parent().siblings(":eq(2)");
		$("#loginName").val(loginNameObj.text());
		$("#loginPwd").val(loginPwdObj.text());
        ...
        $("#myes").click(function(){
            $.ajax({
                ...
                success:function (data) {
                    ...
                    loginNameObj.text($("#loginName").val());
                    loginPwdObj.text($("#loginPwd").val());
                },
                error:function (jqXHR) {
                    alert(jqXHR.status);
                }
